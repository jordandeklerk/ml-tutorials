...
$$
\textcolor{#c85417}{E\bigg[\sum_{t=1}^{\infty} \gamma^{t-1}  r_t | \pi, s_1 \bigg]},
$$
...
1. **State-value function** $\textcolor{#DC143C}{V_{\mathcal{M}}^\pi(s)}$: This tells us the expected total reward if we start in state $s$ and follow policy $\pi$ thereafter:
   ...
   $$
   \textcolor{#DC143C}{V_{\mathcal{M}}^\pi(s) = E\bigg[\sum_{t=1}^{\infty} \gamma^{t-1} r_t | \pi, s_1 = s\bigg]}.
   $$
...
2. **Action-value function** $\textcolor{#1E90FF}{Q_{\mathcal{M}}^\pi(s,a)}$: This is similar, but it considers what happens if we start in state $s$, take a specific action $a$, and then follow policy $\pi$ thereafter:
   ...
   $$
   \textcolor{#1E90FF}{Q_{\mathcal{M}}^\pi(s,a) = E\bigg[\sum_{t=1}^{\infty} \gamma^{t-1} r_t | \pi, s_1 = s, a_1 = a\bigg]}.
   $$
...
<!-- In this approach, the agent learns directly from interactions with the environment, updating its policy or value estimates based on the rewards it receives. It doesn't attempt to predict state transitions or reward functions.

Much of the remainder of this post will be devoted to model-free RL. -->
...
<!-- By building an internal representation of the environment's dynamics, the agent can simulate potential outcomes and plan ahead. This approach allows for more efficient use of experience, as the agent can "imagine" scenarios without actually experiencing them. -->
...
<!-- This approach tends to be more stable as there's a direct correspondence between the policy being followed and the policy being learned. However, it may be less exploratory, as the agent is always following its current best policy.

On-policy methods can be effective in many scenarios, but they may struggle to learn from experiences generated by other policies or historical data. -->
...
<!-- This approach is more flexible in terms of data usage and can be more sample-efficient. It allows the agent to learn about the optimal policy while following an exploratory policy, potentially leading to better exploration of the state space.

While off-policy methods offer greater flexibility, they can be less stable than on-policy methods due to the mismatch between the behavior policy and the target policy. However, when implemented correctly, they can be very powerful, especially in scenarios where gathering new experience is costly or risky. -->

<!-- ## A Brief Primer on Dynamic Programming <a id="a-brief-primer-on-dynamic-programming" style="padding-top: 70px; margin-top: -70px; display: block;"></a>

At its core, dynamic programming is an elegant method for solving complex problems by breaking them down into simpler, more manageable sub-problems. Imagine you're faced with a daunting puzzle. Instead of tackling it all at once, you solve small pieces and gradually combine them to form the complete picture. That's the spirit of dynamic programming.

The genius of this approach lies in its efficiency. As you solve these sub-problems, you store their solutions. Why? Because you'll likely encounter the same sub-problems multiple times on your way to solving the larger problem. By storing the solutions, you avoid redundant computations, dramatically speeding up the process.

### Enter the Bellman Equation <a id="enter-the-bellman-equation" style="padding-top: 70px; margin-top: -70px; display: block;"></a>

The Bellman equation is the mathematical heart of dynamic programming. It's a powerful tool that expresses the value of a decision problem at a certain point in terms of the payoff from some initial choices and the value of the remaining decision problem that results from those choices.

The general form of the Bellman equation looks like this

$$
V(s) = \max_{a \in \mathcal{A}(s)} \big \{f(s, a) + \gamma V(T(s, a)) \big\},
$$

where many of these components will be familiar from our discussion of MDP's:

- $s$ represents our current state
- $\mathcal{A}(s)$ is the set of actions we can take in state $s$
- $a$ is a specific action we might choose
- $f(s, a)$ gives us the immediate reward for taking action $a$ in state $s$
- $T(s, a)$ is our transition function, telling us what state we'll end up in
- $\gamma$ is a discount factor, valuing immediate rewards over future ones
- $V(s)$ is our value function, representing the maximum expected cumulative reward starting from state $s$

In essence, this equation tells us that the value of being in any state is equal to the maximum reward we can get by choosing the best action, plus the discounted value of wherever that action takes us.

### From Theory to Practice <a id="from-theory-to-practice" style="padding-top: 70px; margin-top: -70px; display: block;"></a>

Here are a few practical applications of the Bellman equation and dynamic programming, including the MDP which we will explore in greater detail in the next section.

1. **Shortest Path Problems**: Imagine you're a delivery driver trying to find the quickest route through a city. The Bellman equation for this scenario simplifies to:

   $$
   V(i) = \min_{j \in N(i)} \big \{c_{ij} + V(j) \big \}
   $$

   Here, $c_{ij}$ is the cost (or time) to move from intersection $i$ to $j$, and $N(i)$ is the set of intersections connected to $i$.

2. **The Knapsack Problem**: You're packing for a camping trip, trying to maximize the value of items you bring while staying within a weight limit. The dynamic programming solution uses this recurrence:

   $$
   V(i, w) = \max \big \{V(i-1, w), v_i + V(i-1, w-w_i) \big \}
   $$

   Here, $v_i$ and $w_i$ are the value and weight of item $i$, and $w$ is your remaining weight capacity.

3. **Markov Decision Processes**: The fundamental structure in reinforcement learning! The Bellman equation here can look like:

   $$
   V(s) = \max_a \big \{R(s, a) + \gamma \sum_{s'} P(s'|s, a)V(s') \big \}
   $$

   $R(s, a)$ is the reward function, and $P(s'|s, a)$ gives us the probability of transitioning to state $s'$.

### Solving Bellman Equations <a id="solving-bellman-equations" style="padding-top: 70px; margin-top: -70px; display: block;"></a>

In practice, we often use iterative methods to solve these equations. Two popular approaches are:

1. **Value Iteration**: We start with arbitrary values and iteratively improve our estimates of each state's value.

2. **Policy Iteration**: We alternate between evaluating a policy (a strategy for choosing actions) and improving it based on that evaluation.

These methods converge to optimal solutions, allowing us to tackle complex decision-making problems. -->
...
In reinforcement learning, we're often trying to figure out how good a policy is. We do this by estimating the value functions from the previous section, which tell us the expected future rewards when following a particular policy:
...
These functions are related through the following Bellman equations:
...
Using this operator, we can rewrite the Bellman optimality equation for $Q^{\star}$ in Eq. 3 in a concise form:
...
Before exploring traditional methods for addressing reinforcement learning (RL) challenges, let's consider some alternative configurations of the environment we've set up.
